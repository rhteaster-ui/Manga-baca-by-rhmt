...
export default function MusicPlayer() {
  ...
  // Hide on about + search pages
  const hide =
    pathname === "/about" ||
    pathname === "/search" ||
    (pathname?.startsWith("/search") && pathname?.length > 8);
  if (hide) return null;
  ...
  return (
    <>
      ...
      >
        <button
          onClick={() => !dragData.current.moved && setIsOpen(true)}
        ...
        >
          ...
          <svg className="absolute inset-0" width="54" height="54" viewBox="0 0 54 54" style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle
              ...
              stroke={isPlaying ? "#f59e0b" : "rgba(245,158,11,0.4)"}
      ...
      {isOpen && (
        ...
        >
          <div
            className="w-full max-w-sm mx-4 mb-20 rounded-3xl overflow-hidden fade-in"
          ...
          >
            ...
            {currentTrack ? (
              <div className="px-5 pt-3 pb-5 flex flex-col items-center">
                ...
                />
                <p className="text-sm font-bold text-white text-center line-clamp-1 w-full mb-0.5">
                ...
                <div className="w-full mb-1">
                  <div className="w-full h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                    <div
                      ...
                      style={{ width: `${progress * 100}%`, background: "linear-gradient(to right,#f59e0b,#ef4444)", transition: "width 0.25s linear" }}
                    />
                  </div>
                  <div className="flex justify-between mt-1">
                    <span className="text-[10px]" style={{ color: "#6b7280" }}>{formatTime(currentTime)}</span>
                    <span className="text-[10px]" style={{ color: "#6b7280" }}>{formatTime(duration || currentTrack.trackTimeMillis / 1000)}</span>
                  </div>
                </div>
            ...
            <div className="px-4 pb-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              <form
                onSubmit={(e) => { e.preventDefault(); doSearch(searchInput || "Inni uhibbuka"); }}
            ...
            {tracks.length > 0 && (
              <div className="px-4 pb-3 space-y-0.5 max-h-44 overflow-y-auto hide-scrollbar">
                ...
                {tracks.map((t, i) => (
                  <button
                    key={i}
                    onClick={() => { setCurrentTrack(t); if (isPlaying) setIsPlaying(false); }}
                  ...
                  >
                    ...
                    {currentTrack === t && isPlaying && (
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="#f59e0b" className="flex-shrink-0 animate-pulse">
                        <rect x="3" y="9" width="3" height="12" rx="1"/><rect x="9" y="5" width="3" height="16" rx="1"/><rect x="15" y="7" width="3" height="14" rx="1"/><rect x="21" y="3" width="3" height="18" rx="1"/>
                      </svg>
                    )}
                  </button>
                ))}
            ...
            <button
              ...
              className="w-full py-3 text-xs text-center transition-colors hover:text-white"
